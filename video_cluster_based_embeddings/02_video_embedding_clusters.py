...
def cluster_videos(
    ...
    _save_plot(fig, visualization_dir / "video_tsne_projection.png", "t-SNE Projection of Video Embeddings")
    ...
    ax.grid(True)
    _save_plot(fig, visualization_dir / "video_silhouette_scores.png", "Silhouette Score for Video Clusters")
    ...
    ax.grid(True)
    _save_plot(fig, visualization_dir / "video_elbow_method.png", "Elbow Method for Optimal k (Videos)")

    optimal_k_silhouette = k_range[np.argmax(silhouette_scores)]
    print(f"Optimal video k (Silhouette): {optimal_k_silhouette}")
    try:
        kneedle = KneeLocator(list(k_range), inertia_values, curve="convex", direction="decreasing")
    ...
    optimal_k_video = max(optimal_k_silhouette, optimal_k_elbow, default_optimal_k_video)
    ...
    ax.set_xlabel("t-SNE Dimension 1")
    ax.set_ylabel("t-SNE Dimension 2")
    _save_plot(fig, visualization_dir / "video_tsne_clusters.png", f"t-SNE Projection with {optimal_k_video} Video Clusters")
    ...
    _save_plot(fig, visualization_dir / "video_cluster_sizes.png", f"Video Count per Cluster (k={optimal_k_video})")
    ...
    summary = {
        ...
        "silhouette_scores": {int(k): float(s) for k, s in zip(k_range, silhouette_scores)},
    ...
    print(f"Video cluster analysis complete. Results saved to {visualization_dir} and {transformed_dir}")
...
def calculate_user_cluster_distributions(
    ...
    video_cluster_label_map = dict(zip(video_cluster_map_df['video_id'], video_cluster_map_df['cluster']))
    ...
    df_all_data['engagement_metadata'] = df_all_data.apply(
        lambda x: {
            ...
            'cluster_label': int(x['cluster']) if pd.notna(x['cluster']) else None, # Ensure int
        },
        axis=1
    )
    ...
    df_user_engagement.to_parquet(transformed_dir / "user_engagement_metadata_with_video_clusters.parquet")
    ...
    df_user_engagement["cluster_distributions"] = df_user_engagement['engagement_metadata_list'].apply(get_cluster_distribution)
    df_user_engagement.to_parquet(transformed_dir / "user_engagement_metadata_with_video_clusters_distribution.parquet")
...
def cluster_users(
    ...
    _create_directories(visualization_dir, transformed_dir)
    user_distributions_raw = np.array(df_user_engagement_with_dist["cluster_distributions"].tolist())
    ...
    if process_non_normalized:
        processing_approaches.append({"name": "non_normalized", "data": user_distributions_raw, "scaler": None})
    if process_normalized:
        scaler_user = StandardScaler()
        normalized_user_distributions = scaler_user.fit_transform(user_distributions_raw)
        processing_approaches.append({"name": "normalized", "data": normalized_user_distributions, "scaler": scaler_user})

    for approach in processing_approaches:
        name = approach["name"]
        user_distributions = approach["data"]
        print(f"Processing {name} user distributions... Shape: {user_distributions.shape}")

        # Perform t-SNE
        tsne_user = TSNE(n_components=2, perplexity=tsne_perplexity, n_iter=tsne_n_iter, random_state=42)
        ...
        for k in tqdm(k_range_user, desc=f"User K-Means ({name})"):
            ...
            if len(np.unique(user_cluster_labels_temp)) > 1: # Silhouette score requires at least 2 labels
                score = silhouette_score(user_distributions, user_cluster_labels_temp)
                silhouette_scores_user.append(score)
            else:
                silhouette_scores_user.append(-1) # Assign a low score if only one cluster is formed
            inertia_values_user.append(kmeans_user.inertia_)
        
        optimal_k_user_silhouette = k_range_user[np.argmax(silhouette_scores_user)] if silhouette_scores_user else default_optimal_k_user
        print(f"Optimal user k ({name}, Silhouette): {optimal_k_user_silhouette}")
        try:
            kneedle_user = KneeLocator(list(k_range_user), inertia_values_user, curve="convex", direction="decreasing")
            ...
            print(f"Elbow method failed for {name}, using default k: {optimal_k_user_elbow}")
        
        optimal_k_user = max(optimal_k_user_silhouette, optimal_k_user_elbow, default_optimal_k_user)
        ...
        fig, ax = plt.subplots(figsize=(12, 10))
        scatter = ax.scatter(
            tsne_user_results[:, 0], tsne_user_results[:, 1], c=user_cluster_labels, alpha=0.6, cmap="viridis"
        )
        fig.colorbar(scatter, ax=ax, label="User Cluster")
        _save_plot(fig, visualization_dir / f"tsne_user_clusters_{name}.png", f"t-SNE User Distributions - {name} ({optimal_k_user} clusters)")
        ...
        fig_plotly_user.update_layout(width=1000, height=800, legend_title_text="User Cluster")
        fig_plotly_user.write_html(visualization_dir / f"interactive_tsne_user_clusters_{name}.html")
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.barplot(x=f"user_cluster_{name}", y="user_count", data=user_cluster_stats, ax=ax)
        _save_plot(fig, visualization_dir / f"user_cluster_sizes_{name}.png", f"User Count per Cluster - {name} (k={optimal_k_user})")
        
        all_user_cluster_summaries[name] = {
            "optimal_clusters_silhouette": int(optimal_k_user_silhouette),
            "optimal_clusters_elbow": int(optimal_k_user_elbow) if optimal_k_user_elbow else None,
            "optimal_clusters_used": int(optimal_k_user),
            "silhouette_scores": {int(k): float(s) for k, s in zip(k_range_user, silhouette_scores_user)},
            "inertia_values": {int(k): float(i) for k, i in zip(k_range_user, inertia_values_user)},
            "cluster_sizes": user_cluster_stats.to_dict(orient="records"),
        }

    df_user_engagement_with_dist.to_parquet(transformed_dir / "user_clusters_with_distributions.parquet")
    _save_json(all_user_cluster_summaries, visualization_dir / "user_cluster_analysis_summary.json")
    print(f"User cluster analysis complete. Results saved to {visualization_dir} and {transformed_dir}")
...
def main_video_user_clustering(
    ...
    assert df_input["embedding"].isna().sum() == 0, "Embeddings should not contain NaN values"

    # Prepare video embeddings
    video_embedding_map = df_input.groupby('video_id').agg(
        embedding=('embedding', 'first'),
    ).reset_index() # df_input already has mean of video embedding, therefore the video_id_nunique >= video_embedding_nunique (> condition for the duplicates)
    ...
    video_cluster_label_map_dict = dict(zip(df_video_clusters_map['video_id'], df_video_clusters_map['cluster']))
    _save_json(video_cluster_label_map_dict, video_transformed_dir / "video_cluster_label_map.json")
    print(f"Video cluster label map saved to {video_transformed_dir / 'video_cluster_label_map.json'}")
    ...
    if run_user_clustering_normalized and 'user_cluster_normalized' in df_user_final_clusters.columns:
        ...
        user_cluster_counts = df_user_final_clusters['user_cluster_normalized'].value_counts()
        if not user_cluster_counts.empty:
            ...
            engagement_lists = users_in_cluster['engagement_metadata_list'].sum() # Concatenate all lists
            
            if engagement_lists: # Check if there's any engagement data
                engaged_video_ids = [item['video_id'] for item in engagement_lists if 'video_id' in item]
                ...
                print(f"No engagement metadata found for users in cluster {example_cluster_id}")
        else:
