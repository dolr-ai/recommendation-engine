    ...
    with _redis_lock:
        if _redis_client is None:
            try:
                ...
                logger.error(
                    f">>> REDIS CONNECTION INFO: host={host}, port={port}, authkey={'SET' if authkey else 'NOT SET'}"
                )
    ...
    if uncached_video_ids:
            ...
            if use_redis_mappings and not metadata_df.empty:
                ...
                if redis_mappings:
                    ...
                    if canister_mask.any():
                        metadata_df.loc[canister_mask, 'canister_id'] = metadata_df.loc[canister_mask, 'redis_canister_id']

                        # Log summary of canister_id updates
                        updated_count = canister_mask.sum()
                        logger.info(f"Applied Redis canister_id mappings to {updated_count} videos")
                    ...
                    if post_mask.any():
                        ...
                        if post_id_as_string:
                            ...
                            def convert_post_id_string(redis_post_id):
                                if is_valid_uint64(redis_post_id):
                                    ...
                                    logger.warning(f"Invalid post_id for v2 API from Redis: {redis_post_id}")
                                    return None

                            metadata_df.loc[post_mask, 'converted_post_id'] = metadata_df.loc[post_mask, 'redis_post_id'].apply(convert_post_id_string)
                            valid_conversions = metadata_df['converted_post_id'].notna()
                            metadata_df.loc[post_mask & valid_conversions, 'post_id'] = metadata_df.loc[post_mask & valid_conversions, 'converted_post_id']
                        else:
                            ...
                            def convert_post_id_int(redis_post_id):
                                try:
                                    ...
                                    logger.warning(f"Could not convert post_id to int from Redis: {redis_post_id}")
                                    return None

                            metadata_df.loc[post_mask, 'converted_post_id'] = metadata_df.loc[post_mask, 'redis_post_id'].apply(convert_post_id_int)
                            valid_conversions = metadata_df['converted_post_id'].notna()
                            metadata_df.loc[post_mask & valid_conversions, 'post_id'] = metadata_df.loc[post_mask & valid_conversions, 'converted_post_id']
                        ...
                        if updated_count > 0:
                            ...
                            logger.info(f"Applied Redis post_id mappings to {updated_count} videos ({post_type})")

                    # Clean up temporary columns
                    metadata_df = metadata_df.drop(columns=['redis_canister_id', 'redis_post_id'], errors='ignore')
            ...
            try:
                ...
                for item in metadata_list:
                    ...
                    if video_id in new_metadata:
                        ...
                        existing_metadata["nsfw_probability"] = item.get("nsfw_probability", 0.0)
                ...
                if redis_preserved_count > 0:
                    logger.info(f"🔒 Preserved Redis mappings for {redis_preserved_count} videos while adding NSFW data")
                if bigquery_added_count > 0:
                    logger.info(f"📊 Added {bigquery_added_count} videos using original BigQuery data")
        ...
        if fetch_nsfw_probabilities and new_metadata:
            ...
            if nsfw_probabilities:
                ...
                if updated_count > 0:
                    logger.info(f"Updated NSFW probabilities for {updated_count} videos")
        ...
        if new_metadata:
            with _cache_lock:
                ...
                logger.info(f"Updated cache for {len(new_metadata)} video metadata records")
