"""
History Cache Refresh DAG

This DAG triggers the recommendation-history Cloud Run service to refresh the user history cache.
...
IMAGE_NAME = "recommendation-history"  # Matches the image name in GitHub workflow
REPOSITORY = "recommendation-engine-registry"  # Hardcoded to match GitHub workflow

# Status variable name
STATUS_VARIABLE = "cache_refresh_history_completed"
...
def generate_job_name(**kwargs):
    ...
    if execution_date:
        # Format: recommendation-history-job-YYYYMMDD-HHMMSS
    ...
    job_name = f"recommendation-history-job-{timestamp}"
...
def initialize_status_variable(**kwargs):
    """Initialize the cache_refresh_history_completed status variable to False."""
    try:
...
def set_status_completed(**kwargs):
    """Set the cache_refresh_history_completed status variable to True."""
    try:
...
with DAG(
    ...
    description="Refresh Recommendation History Cache",
    schedule_interval="0 3 * * *",  # Daily at 3 AM (after other caches)
    catchup=False,
    tags=["cache_refresh", "history"],
) as dag:
                ...
                "containers": [
                    {
                        ...
                        "command": ["python"],
                        "args": ["-m", "src.history.set_history_items"],
    ...
    run_job = CloudRunExecuteJobOperator(
        task_id="task-run_history_refresh",
        project_id=PROJECT_ID,
        region=REGION,
        job_name=job_name,
    )
