"""
Reported Items Cache Refresh DAG

This DAG triggers the recommendation-reported-items Cloud Run service to refresh the user reported items cache.
...
DAG_ID = "cache_refresh_reported_items"
...
IMAGE_NAME = (
    "recommendation-reported-items"  # Matches the image name in GitHub workflow
)
REPOSITORY = "recsys-repository"  # Hardcoded to match GitHub workflow

# Status variable name
STATUS_VARIABLE = "cache_refresh_reported_items_completed"
    ...
    def send(self, context, status):
        ...
        card = {
            "cards": [
                {
                    "header": {
                        "title": f"Reported Items Alert: {config['title']}",
                        "subtitle": f"{dag_id}",
...
def generate_job_name(**kwargs):
    ...
    if execution_date:
        # Format: recommendation-reported-items-job-YYYYMMDD-HHMMSS
    ...
    job_name = f"rs-reported-items-cache-update-{timestamp}"
...
def initialize_status_variable(**kwargs):
    """Initialize the cache_refresh_reported_items_completed status variable to False."""
    try:
...
def set_status_completed(**kwargs):
    """Set the cache_refresh_reported_items_completed status variable to True."""
    try:
...
with DAG(
    ...
    description="Refresh Recommendation Reported Items Cache",
    schedule_interval=None,
    catchup=False,
    tags=["cache_refresh", "reported_items"],
    ...
    run_job = CloudRunExecuteJobOperator(
        task_id="task-run_reported_items_refresh",
        project_id=PROJECT_ID,
        region=REGION,
        job_name=job_name,
