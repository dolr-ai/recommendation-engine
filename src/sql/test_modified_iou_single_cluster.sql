...
WITH
    ...
    -- Get count of videos in base_data (for debugging)
    video_count AS (
        SELECT
    ...
    -- Get count of pairs before filtering (for debugging)
    pair_count_before AS (
        SELECT COUNT(*) AS count_pairs FROM all_pairs_with_iou
    ),
    ...
    -- Get count of final candidates (for debugging)
    candidate_count AS (
        SELECT COUNT(*) AS count_candidates FROM final_candidates
    ),
    -- Generate summary statistics for cluster 1 only
    cluster_summary AS (
        SELECT
            1 AS cluster_id,
            (SELECT count_candidates FROM candidate_count) AS number_of_candidates,
            (
                SELECT COUNT(*)
    ...
    -- Output the debugging information along with results
SELECT
    'DEBUG INFO' AS info_type,
    (SELECT distinct_videos FROM video_count) AS video_count,
    (SELECT count_pairs FROM pair_count_before) AS pair_count,
    (SELECT p95_threshold FROM percentile_calc) AS p95_threshold,
    (SELECT count_candidates FROM candidate_count) AS candidate_count,
    0 AS top_5pct_count,
    0 AS total_pairs
UNION ALL
...
SELECT
    ...
    number_of_videos,
    number_of_users,
    0 AS top_5pct_count,  -- Adding extra columns to match first query
    0 AS total_pairs      -- Adding extra columns to match first query
FROM
    cluster_summary;

-- Uncomment below to see all pairs with their IoU scores
/*
SELECT
    cluster_id,
    video_id_x,
