...
CREATE OR REPLACE FUNCTION `jay-dhanwant-experiments.stage_test_tables.video_ids_to_urls`(input_video_ids ARRAY<STRING>)
RETURNS ARRAY<STRUCT<video_id STRING, yral_url STRING>>
AS (
  ARRAY(
    SELECT STRUCT(
      input_video_id as video_id,
      CONCAT('https://yral.com/hot-or-not/', vi.canister_id, '/', vi.post_id) as yral_url
    )
    FROM UNNEST(input_video_ids) as input_video_id
    LEFT JOIN `jay-dhanwant-experiments.stage_tables.stage_video_index` vi
      ON `jay-dhanwant-experiments.stage_test_tables.extract_video_id`(vi.uri) = input_video_id
  )
);
