...
INSERT INTO `jay-dhanwant-experiments.stage_tables.video_embedding_average` (video_id, avg_embedding)
WITH missing_videos AS (
    SELECT DISTINCT `jay-dhanwant-experiments.stage_test_tables.extract_video_id`(uri) as video_id
    FROM `jay-dhanwant-experiments.stage_tables.stage_video_index`
    WHERE `jay-dhanwant-experiments.stage_test_tables.extract_video_id`(uri) IS NOT NULL
      AND `jay-dhanwant-experiments.stage_test_tables.extract_video_id`(uri) NOT IN (
        SELECT video_id
...
flattened_embeddings AS (
    SELECT
        `jay-dhanwant-experiments.stage_test_tables.extract_video_id`(s.uri) as video_id,
        pos,
        embedding_value
    FROM `jay-dhanwant-experiments.stage_tables.stage_video_index` s
    INNER JOIN missing_videos m ON `jay-dhanwant-experiments.stage_test_tables.extract_video_id`(s.uri) = m.video_id,
    UNNEST(embedding) AS embedding_value WITH OFFSET pos
),
averaged_by_position AS (
    SELECT
