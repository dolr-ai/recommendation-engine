...
Output:
- Uploads the user embeddings to BigQuery table recsys_user_embeddings
- Uploads the flattened engagement metadata to BigQuery table recsys_user_cluster_interaction
...
def upload_to_bigquery(df_spark, gcp_utils, dataset_id, table_id, credentials_path):
    ...
    df_spark.write.format("bigquery").option("table", table_ref).option(
        "temporaryGcsBucket", "yral-ds-dataproc-bucket"
...
def main():
    ...
    embeddings_table_id = "recsys_user_embeddings"
    clusters_table_id = "recsys_user_cluster_interaction"
